...
def is_safe(x, y, n, board) -> bool:
    ...
    if board[x][y] != -1:
        # if its anything other than -1
        # which we explicitly inited it with
        # i.e. the knight has already visited it
        # and marked it with `pos`
        return False
...
def solve_knights_tour(n, board, curr_x, curr_y, pos):
    ...
    for i in range(8):  # max 8 moves for any given position
        ...
        # Note that the following function call is also where the
        # value of `pos` is getting incremented
